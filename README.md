...
Combining multiple audio features can improve music tagging performance, but typical explicit or implicit feature fusions in deep learning approaches lack interpretability. We propose a Genetic Programming (GP) pipeline to automatically evolve interpretable composite features by mathematically combining base music features, thereby capturing synergistic interactions among them. This approach attains some representational benefits akin to deep learning feature fusion without sacrificing transparency. Experiments on the MTG-Jamendo and GTZAN datasets show that our method consistently improves tagging performance across base feature sets in different abstraction levels. Notably, performance gains emerge within the first few hundred GP evaluations, indicating that effective feature combinations are found with a modest search budget. The evolved top feature expressions include linear, nonlinear, and conditional forms, and contain relatively low-complexity solutions that align with the GP's parsimony pressure. Analyzing these composite features reveals which feature interactions and transformations tend to be most beneficial for tagging, providing insights that are difficult to obtain from black-box deep models.

### Authors

**Chenhao Xue¹**, Weitao Hu², Puyu Wang¹, Joyraj Chakraborty¹, Zhijin Guo¹, Kang Li¹, Tianyu Shi³, Martin Reed⁴, Nikolaos Thomos⁴
